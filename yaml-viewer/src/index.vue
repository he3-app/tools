<template>
  <h-split-pane-layout>
    ...
    <template #right>
      <json-viewer :value="outputValue" :expand-depth="5" sort/>
    </template>
  </h-split-pane-layout>
</template>
...
import {yamlExample, isYaml} from 'he3-common'
...
const inputText = ref<string>(yamlExample)
...
async function handleAnalyze() {
  try {
    outputValue.value = YAML.parse(inputText.value)
    console.log(outputValue.value)
  } catch (e) {
    outputValue.value = e.message
  }
}
</script>
...
<!-- vue-json-viewer 的 css， 做了暗黑模式的适配-->
<style>
.jv-node {
  position: relative;
}
...
.jv-container.jv-light .jv-ellipsis {
  color: var(--active-extension-title-color);
