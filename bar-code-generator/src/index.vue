  ...
  <h-horizontal-layout mode="middle">
    <template #left>
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
        ...
        >
          <a-input-group>
            <a-row :gutter="10">
              ...
              <a-col :span="6">
                <a-input
                  v-model:value="bOpts.background"
                  type="color"
                  @change="handleGenerate"
                />
              </a-col>
        ...
        <a-form-item :label="t('label.showText')">
          <a-switch v-model:checked="tOpts.show" @change="handleGenerate" />
        </a-form-item>

        <a-form-item
          v-if="tOpts.show"
          :label="t('label.textAlign')"
        >
          <a-radio-group
    ...
    <template #right>
      ...
      <div v-show="isValid" class="output">
        <canvas
          id="he3-barcode-canvas"
          ref="barCodeOutput"
          class="barcode-output"
        ></canvas>
      </div>
      <div v-show="!isValid">
...
function fillExampleAndGen() {
  let example = EXAMPLE_DATA.get(bOpts.value.format);
