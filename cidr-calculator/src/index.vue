  ...
  <H-single-layout>
    <a-form ref="formRef" :model="formModel" :rules="rules">
      <a-formItem name="cidr" :label="t('cidrAddress')">
        <a-inputSearch v-model:value="formModel.cidr" :enter-button="t('calculate')" @search="onCalculate" />
      </a-formItem>
    </a-form>
    ...
    <a-table :columns="columns" :data-source="dataSource" :show-header="false" :pagination="false">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'action'">
          <h-textCopy v-if="record.value" type="link" :content="record.value" />
        </template>
      </template>
    </a-table>
  </H-single-layout>
...
const props = defineProps<{
  ...
  searchValue?: JudgeType
  renderOnSearch?: boolean
}>()
const emit = defineEmits(['acceptClipboardCb'])
onMounted(() => {
  // auto fill
  if (props.clipboardValue?.isCidr()) {
    formModel.value.cidr = props.clipboardValue?.getOriginData()
...
const ipv4Netmask = function (mask: number) {
  ...
  const maskArr = []
  for (let i = 0; i < 4; i++) {
    maskArr.push(parseInt(maskBin.slice(i * 8, i * 8 + 8), 2))
  }
  return maskArr.join('.')
}
...
const ipv6Netmask = function (mask: number) {
  ...
  const maskArr = []
  for (let i = 0; i < 8; i++) {
    maskArr.push(parseInt(maskBin.slice(i * 16, i * 16 + 16), 2).toString(16))
  }
  return maskArr.join(':')
}
</script>
